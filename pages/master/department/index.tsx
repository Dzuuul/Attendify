...
import { getData } from "../../api/master/department/list";
import { masterEmployee } from "../../api/master/index";
import { useApp } from "../../../context/AppContext";
import { IState } from "../../../interfaces/department.interface";
...
const Department = (props: any, { fallback }: any) => {
    ...
    const url = `/api/master/department/list?page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}`
    const { data, error } = useSWR(url)

    const deleteJobdata = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/department/delete?&submit=${data}`)
    }
    ...
    const submitUpdate = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/department/update?&submit=${data}`)
    }

    const submit = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/department/add?&submit=${data}`)
    }
    ...
    return (
        <>
            <SWRConfig value={{ fallback }}>
                <PageHeader
                    title="Department Data Management"
                    extra={[
                ...
                <Modals
                    ...
                    header={states.typeModal == "Add" ? "Add Department" : "Update Department"}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    return {
        props: {
            fallback: {
                '/api/master/department/list': JSON.parse(JSON.stringify(data))
            },
