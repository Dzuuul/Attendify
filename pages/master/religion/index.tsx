...
import { getData } from "../../api/master/religion/list";
import { useApp } from "../../../context/AppContext";
import { IState } from "../../../interfaces/religion.interface";
...
const Religion = (props: any, { fallback }: any) => {
    ...
    const url = `/api/master/religion/list?page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}`
    const { data, error } = useSWR(url)

    const deleteReligion = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/religion/delete?&submit=${data}`)
    }
    ...
    const submitUpdate = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/religion/update?&submit=${data}`)
    }

    const submit = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/religion/add?&submit=${data}`)
    }
                    ...
                    <>
                        <Space size="middle">
                            ...
                            {states.access.m_delete == 1 ?
                                <a className={"link"}
                                    onClick={() => showDeleteConfirm({ onOk: (() => deleteReligion(record)) })}
                                >
                                    Delete
                                </a>
                                : null}
    ...
    return (
        <>
            <SWRConfig value={{ fallback }}>
                <PageHeader
                    title="Religion Data Management"
                    extra={[
                ...
                <Modals
                    ...
                    header={states.typeModal == "Add" ? "Add Religion" : "Edit Religion"}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    return {
        props: {
            fallback: {
                '/api/master/religion/list': JSON.parse(JSON.stringify(data))
            },
...
Religion.getLayout = function getLayout(page: ReactElement) {
    return (
