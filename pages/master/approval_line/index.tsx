...
import { getData } from "../../api/master/approval_line/list";
import { masterEmployee, masterDivision } from "../../api/master/index";
import { useApp } from "../../../context/AppContext";
import { IState } from "../../../interfaces/approval_line.interface";
...
const ApprovalLine = (props: any, { fallback }: any) => {
    ...
    const url = `/api/master/approval_line/list?page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}`
    const { data, error } = useSWR(url)

    const deleteJobdata = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/approval_line/delete?&submit=${data}`)
    }
    ...
    const submitUpdate = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/approval_line/update?&submit=${data}`)
    }

    const submit = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/approval_line/add?&submit=${data}`)
    }
    ...
    return (
        <>
            <SWRConfig value={{ fallback }}>
                <PageHeader
                    title="Approval Line Data Management"
                    extra={[
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    return {
        props: {
            fallback: {
                '/api/master/approval_line/list': JSON.parse(JSON.stringify(data))
            },
...
ApprovalLine.getLayout = function getLayout(page: ReactElement) {
    return (
