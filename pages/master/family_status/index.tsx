...
import { getData } from "../../api/master/family_status/list";
import { useApp } from "../../../context/AppContext";
import { IState } from "../../../interfaces/family_status.interface";
...
const FamilyStatus = (props: any, { fallback }: any) => {
    ...
    const url = `/api/master/family_status/list?page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}`
    const { data, error } = useSWR(url)

    const deleteFamily = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/family_status/delete?&submit=${data}`)
    }
    ...
    const submitUpdate = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/family_status/update?&submit=${data}`)
    }

    const submit = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/family_status/add?&submit=${data}`)
    }
                    ...
                    <>
                        <Space size="middle">
                            ...
                            {states.access.m_delete == 1 ?
                                <a className={"link"}
                                    onClick={() => showDeleteConfirm({ onOk: (() => deleteFamily(record)) })}
                                >
                                    Delete
                                </a>
                                : null}
    ...
    return (
        <>
            <SWRConfig value={{ fallback }}>
                <PageHeader
                    title="Family Status Data Management"
                    extra={[
                ...
                <Modals
                    ...
                    header={states.typeModal == "Add" ? "Add Family Status" : "Edit Family Status"}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    return {
        props: {
            fallback: {
                '/api/master/family_status/list': JSON.parse(JSON.stringify(data))
            },
...
FamilyStatus.getLayout = function getLayout(page: ReactElement) {
    return (
