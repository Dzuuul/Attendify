...
import { getData } from "../../api/master/marriage_status/list";
import { useApp } from "../../../context/AppContext";
import { IState } from "../../../interfaces/marriage_status.interface";
...
const Marriage = (props: any, { fallback }: any) => {
    ...
    const url = `/api/master/marriage_status/list?page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}`
    const { data, error } = useSWR(url)

    const deleteMarriage = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/marriage_status/delete?&submit=${data}`)
    }
    ...
    const submitUpdate = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/marriage_status/update?&submit=${data}`)
    }

    const submit = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/marriage_status/add?&submit=${data}`)
    }
                    ...
                    <>
                        <Space size="middle">
                            ...
                            {states.access.m_delete == 1 ?
                                <a className={"link"}
                                    onClick={() => showDeleteConfirm({ onOk: (() => deleteMarriage(record)) })}
                                >
                                    Delete
                                </a>
                                : null}
    ...
    return (
        <>
            <SWRConfig value={{ fallback }}>
                <PageHeader
                    title="Marriage Status Data Management"
                    extra={[
                ...
                <Modals
                    ...
                    header={states.typeModal == "Add" ? "Add Marriage Status" : "Edit Marriage Status"}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    return {
        props: {
            fallback: {
                '/api/master/marriage_status/list': JSON.parse(JSON.stringify(data))
            },
...
Marriage.getLayout = function getLayout(page: ReactElement) {
    return (
