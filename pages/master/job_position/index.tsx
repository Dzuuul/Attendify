...
import { getData } from "../../api/master/job_position/list";
import { useApp } from "../../../context/AppContext";
import { IState } from "../../../interfaces/job_position.interface";
...
const JobPosition = (props: any, { fallback }: any) => {
    ...
    const url = `/api/master/job_position/list?page=${states.data.currentPage}&row=${states.data.dataPerPage}&column=${states.filter.columns}&direction=${states.filter.directions}`
    const { data, error } = useSWR(url)

    const deleteJoblevel = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/job_position/delete?&submit=${data}`)
    }
    ...
    const submitUpdate = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/job_position/update?&submit=${data}`)
    }

    const submit = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/job_position/add?&submit=${data}`)
    }
    ...
    return (
        <>
            <SWRConfig value={{ fallback }}>
                <PageHeader
                    title="Job Position Data Management"
                    extra={[
                ...
                <Modals
                    ...
                    header={states.typeModal == "Add" ? "Add Job Position" : "Edit Job Position"}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    return {
        props: {
            fallback: {
                '/api/master/job_position/list': JSON.parse(JSON.stringify(data))
            },
...
JobPosition.getLayout = function getLayout(page: ReactElement) {
    return (
