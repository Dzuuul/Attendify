...
import { save, edit } from "../../api/master/request_type/list";
import { useApp } from "../../../context/AppContext";

const ProcessShift = (props: any) => {
    ...
    useEffect(() => {
        ...
        router.push('/master/request_type')
    },[])
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    const trueRole = await pageCheck(session.username, '/master/request_type')
    
    if (trueRole.length < 1 || 
        (query.type == "add" && trueRole[0].m_insert == 0) ||
        (query.type == "modify" && trueRole[0].m_update == 0)
    ) {
        return {
            redirect: {
                destination: "/403",
...
ProcessShift.getLayout = function getLayout(page: ReactElement) {
    return (
