...
import { getData } from "../../api/master/request_type/list";
import { useApp } from "../../../context/AppContext";
import { IState } from "../../../interfaces/request_type.interface";
...
const Shift = (props: any, { fallback }: any) => {
    ...
    const url = `/api/master/request_type/list?page=${states.data.currentPage}&row=${states.data.dataPerPage}&key=${states.filter.key}&column=${states.filter.columns}&direction=${states.filter.directions}`
    ...
    const submitUpdate = async (param: any) => {
        ...
        router.push(`/master/request_type/modify?&submit=${data}`)
    }

    const submit = async (param: any) => {
        const data = Buffer.from(JSON.stringify(param)).toString('base64');
        router.push(`/master/request_type/add?&submit=${data}`)
    }
    ...
    useEffect(() => {
        var columns: any = [
            ...
            },
            {
                ...
                dataIndex: "tipe",
                key: "tipe",
                render: (text: any, record: any) => ( text == 1 ? 'HADIR' : text == 2 ? 'TIDAK HADIR' : 'TIDAK POTONG CUTI' )
            },
            {
                title: "Need Approval",
                dataIndex: "need_apprv",
                key: "need_apprv",
    ...
    return (
        <>
            <SWRConfig value={{ fallback }}>
                <PageHeader
                    title="Type of Request Time Off Data Management"
                    extra={[
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
    ...
    return {
        props: {
            fallback: {
                '/api/master/request_type/list': JSON.parse(JSON.stringify(data))
            },
