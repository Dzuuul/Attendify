...
export const listMonthly = (id: number, startDate: string, endDate: string, coalesce: any, left: any, group: any, isBiggerThanToday: boolean) => {
    const syntax = `
    SELECT 
    ${coalesce}
    FROM mst_employee B
    ...
    JOIN mst_department C ON B."deptId" = C.id 
    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance LEFT JOIN mst_request_type ON attendance.check_type = mst_request_type.id where (mst_request_type.tipe = 1) AND DATE(check_in) BETWEEN '${(startDate)}' AND '${(endDate)}' GROUP BY "employeeId") D ON D."employeeId" = B.id 
    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance LEFT JOIN mst_request_type ON attendance.check_type = mst_request_type.id where DATE(check_in) BETWEEN '${(startDate)}' AND '${(endDate)}' AND mst_request_type.is_cuti = 1 GROUP BY "employeeId") E ON E."employeeId" = B.id 
    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance where DATE(check_in) BETWEEN '${(startDate)}' AND '${(endDate)}' AND check_type = 9 GROUP BY "employeeId") F ON F."employeeId" = B.id 
    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance where DATE(check_in) BETWEEN '${(startDate)}' AND '${(endDate)}' AND check_type = 8 GROUP BY "employeeId") G ON G."employeeId" = B.id 
    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance where check_type = 8 AND DATE(check_in) BETWEEN '${(startDate)}' AND '${(endDate)}' GROUP BY "employeeId") H ON H."employeeId" = B.id 

    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance LEFT JOIN mst_request_type ON attendance.check_type = mst_request_type.id where (mst_request_type.tipe = 1) AND DATE(check_in) BETWEEN '${(startDate)}' AND ${isBiggerThanToday ? 'CURRENT_DATE': `'${endDate}'` } GROUP BY "employeeId") I ON I."employeeId" = B.id 
    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance LEFT JOIN mst_request_type ON attendance.check_type = mst_request_type.id where DATE(check_in) BETWEEN '${(startDate)}' AND ${isBiggerThanToday ? 'CURRENT_DATE': `'${endDate}'` } AND mst_request_type.is_cuti = 1 GROUP BY "employeeId") J ON J."employeeId" = B.id
    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance where DATE(check_in) BETWEEN '${(startDate)}' AND ${isBiggerThanToday ? 'CURRENT_DATE': `'${endDate}'` } AND check_type = 8 GROUP BY "employeeId") M ON M."employeeId" = B.id
    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance where DATE(check_in) BETWEEN '${(startDate)}' AND ${isBiggerThanToday ? 'CURRENT_DATE': `'${endDate}'` } AND check_type = 9 GROUP BY "employeeId") K ON K."employeeId" = B.id
    LEFT JOIN (SELECT "employeeId", COUNT(1) from attendance where check_type = 8 AND DATE(check_in) BETWEEN '${(startDate)}' AND ${isBiggerThanToday ? 'CURRENT_DATE': `'${endDate}'` } GROUP BY "employeeId") L ON L."employeeId" = B.id
