        ...
        if (checkDuplicate.length > 0) {
            return {
                error: {
                    type: 'warning',
                    message: 'warning',
                    description: 'Division Already Exist'
                }
            }
        }

        //Dept hold on
        //const role: any = await model.findDept(param);
        //if (role.length < 1) {
        //    return {
        //        error: {
        //            type: 'warning',
...
export const deleteDivision = async (param: formModal, session: ISession) => {
    try {
        ...
        await model.startTransaction()
        await model.deleteDivision(param, session)
        await model.commitTransaction()
        return 'ok'
    } catch (error) {
        await model.rollback()
