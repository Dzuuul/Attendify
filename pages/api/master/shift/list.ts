...
export const save = async (param: formModal, session: ISession) => {
    try {
        param.description = param.description.toUpperCase();
        //cek duplicate
...
export const edit = async (param: formModal, session: ISession) => {
    try {
        ...
        await model.startTransaction()
        const checkShiftDet: any = await model.findShiftDet(param)
        
        if (param.status == 0) {
            if (checkShiftDet.length > 0) {
                await model.updateShiftDet(checkShiftDet[0].id, param.status, session)
            }
        }

        if (param.status == 1) {
            if (checkShiftDet.length > 0) {
                const data = await model.updateShiftDet(checkShiftDet[0].id, param.status, session)
            }
        }
