...
export const save = async (param: any, session: ISession) => {
    try {
        //cek duplicate
...
export const deleteEducation = async (param: formModal, session: ISession) => {
    try {
        ...
        await model.startTransaction()
        await model.deleteEducation(param, session)
        await model.commitTransaction()
        return 'ok'
    } catch (error) {
        await model.rollback()
