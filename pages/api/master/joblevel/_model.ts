...
export const list = (params: IParams) => {
    ...
    FROM mst_job_level A
    WHERE A.is_deleted = 0`
    return exeQuery(syntax, [])
}

export const save = (param: formModal, session: ISession) => {
    const syntax = `INSERT INTO mst_job_level (description, status, "createdById") VALUES ($1,$2,$3)`;
    return exeQuery(syntax, [param.description || '', param.status || '', session.id])
}

export const update = (param: formModal, session: ISession) => {
    const syntax = `UPDATE mst_job_level SET description = $1, status = $2, "updatedById" = $3 WHERE id = $4`;
    return exeQuery(syntax, [param.description, param.status, session.id, param.id || ''])
}

export const deleteJoblevel = (param: formModal, session: ISession) => {
    const syntax = `UPDATE mst_job_level SET is_deleted = 1, "deletedById" = $1 WHERE id = $2`;
    return exeQuery(syntax, [session.id, param.id || ''])

    //const syntax = `DELETE FROM mst_job_level WHERE id = $1`;
    //return exeQuery(syntax, [param.id || ''])
}

export const findOne = (param: formModal) => {
    const syntax = `SELECT id FROM mst_job_level A WHERE A.description = $1 AND A.is_deleted = 0`;
...
export const findDuplicate = (param: formModal) => {
    const syntax = `SELECT id FROM mst_job_level A WHERE A.description = $1 AND A.id != $2`;
