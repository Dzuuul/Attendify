...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const {file, remType, desc, reqDate, remTitle, address, userId, items} = req.body
    ...
    const addRequest: any = await model.requestReimburse(empId, reqDate, amounte, desc, remType, remTitle, address, session)
    ...
    for(let pijet = 0; pijet < file.length; pijet++) {
        ...
        const add: any = await model.addAttachment(empId, addRequest[0].id, apiImage, session)
    }
    ...
    if (ifHead.length < 1) {
      ...
      let addRq = await model.addReqApproval(session, headHRD[0].headId, addRequest[0].id, 1)
      if(addRq.length < 1) {
        return res.send({error: "error", message: 'No approval line detected.'})
      }
  ...
  if (ifHead[0].divhead == 1 && ifHead[0].depthead == 0) {
    if (ifHead[0].head) {      
      const getDeptHead = ifHead[0].head  
          let addRq = await model.addReqApproval(session, getDeptHead, addRequest[0].id, 1)
          if(addRq.length < 1) {
            return res.send({error: "error", message: 'No approval line detected.'})
          }
          await model.commitTransaction()
