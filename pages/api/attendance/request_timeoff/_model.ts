...
export const list = (param: IPagination) => {
    ...
    WHERE A.is_deleted = 0 AND B.tipe != 1 AND A."employeeId" = '${param.employeeId}' ${dateSummary(param.startDate || "", param.endDate || "")}${keyWhere(param.key)}
    GROUP BY A.id, B.id, F.id ${orderBy(param)}`
